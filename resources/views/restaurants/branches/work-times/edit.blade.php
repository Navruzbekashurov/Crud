...
@section('content')
    <div class="max-w-2xl mx-auto mt-10 bg-white p-6 rounded-lg shadow">
        <h1 class="text-2xl font-bold mb-6 text-center">Edit Weekly Work Time</h1>

        <form method="POST"
              action="{{ route('restaurants.branches.work-times.update', ['restaurant' => $restaurant->id, 'branch' => $branch->id]) }}">
            @csrf
            @method('PUT')
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4 items-center">
                        <div>
                            ...
                            <input type="time" name="work_times[{{ $index }}][open_time]" id="open_time_{{ $index }}"
                                   value="{{ old("work_times.$index.open_time", $existing?->open_time) }}"
                        ...
                        <div>
                            ...
                            <input type="time" name="work_times[{{ $index }}][close_time]" id="close_time_{{ $index }}"
                                   value="{{ old("work_times.$index.close_time", $existing?->close_time) }}"
                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-indigo-200">
                        </div>
                        ...
                        <div class="flex items-center mt-5">
                            <input type="checkbox" name="work_times[{{ $index }}][day_off]" id="day_off_{{ $index }}"
                                   value="1"
                                   {{ old("work_times.$index.day_off", $existing?->day_off) ? 'checked' : '' }}
                                   class="mr-2">
            ...
            <div class="flex justify-end">
                ...
                <button type="submit"
                        class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm">Update Schedule
                </button>
            </div>
        </form>
    </div>

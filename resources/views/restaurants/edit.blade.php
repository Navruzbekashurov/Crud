...
@section('content')
    <div class="max-w-md mx-auto mt-10 bg-white p-6 rounded-lg shadow">
        <h1 class="text-2xl font-bold mb-6 text-center">Edit Restaurant</h1>
        ...
        <form action="{{ route('restaurants.update', $restaurant->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="address" id="address" rows="3"
                          class="w-full mt-1 p-2 border border-gray-300 rounded-md focus:ring focus:ring-indigo-200">{{ old('address', $restaurant->address) }}</textarea>
            </div>

            <div>
                <label for="number" class="block text-sm font-medium text-gray-700">Founder Id</label>
                <input type="number" name="founder_id" id="founder_id"
                       value="{{ old('founder_id', $restaurant->founder_id) }}"
                       class="w-full mt-1 p-2 border border-gray-300 rounded-md focus:ring focus:ring-indigo-200">
            </div>

            <div>
                <label for="employee_numbers" class="block text-sm font-medium text-gray-700">Employee Numbers</label>
                <input type="number" name="employee_numbers" id="employee_numbers"
                       value="{{ old('employee_numbers', $restaurant->employee_numbers) }}"
                       class="w-full mt-1 p-2 border border-gray-300 rounded-md focus:ring focus:ring-indigo-200">
            </div>
            ...
            <div>
                ...
                <input type="date" name="founded_at" id="founded_at"
                       value="{{ old('founded_at', $restaurant->founded_at) }}"
                       class="w-full mt-1 p-2 border border-gray-300 rounded-md focus:ring focus:ring-indigo-200">
            </div>
